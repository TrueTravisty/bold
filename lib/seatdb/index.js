...
function getCharacterInfoAddComments(id, callback) {
    ...
    getCharacterInfoAddMain(id, function(err, character) {
        ...
        pool.getConnection(function(err, connection) {
            ...
            var query = "SELECT * FROM bold_roster_comment WHERE characterID=" + id  + " ORDER BY updated_at DESC";
            connection.query(query, function(err, rows) {
                connection.release();
...
function getCharacterComments(id, callback) {
    var query = "SELECT * FROM bold_roster_comment WHERE characterID=" + id  + " AND deleted=0 ORDER BY updated_at DESC";
    runQueryAndReturnResult(query, callback);
}

function runQueryAndReturnResult(query, callback) {
    //console.log(query);
    if (!pool) callback ("Missing SeAT DB")
    pool.getConnection(function(err, connection) {
...
function addCharacterComment(characterId, userName, comment, callback) {
    var query = "INSERT INTO bold_roster_comment (characterID, user, text) VALUES (" + characterId + 
                    ", \"" + userName + "\", \"" + comment + "\")";
    runQueryAndReturnResult(query, callback);
}

function deleteCharacterComment(id, callback) {
    var query = "update bold_roster_comment SET deleted=1 WHERE id=" + id;
    runQueryAndReturnResult(query, callback);
}
