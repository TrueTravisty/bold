...
function getAllCharacterKeys(characterID, callback) {
    var query = function(connection) {
        return "SELECT DISTINCT aac.keyID, sk.isOk, sk.lastError, sp.personID  " +
               "FROM seat.account_apikeyinfo_characters as aac " +
               "INNER JOIN seat.account_apikeyinfo as aa ON aac.keyID = aa.keyID " +
               "LEFT JOIN seat.seat_keys as sk ON aac.keyID = sk.keyID " +
               "LEFT JOIN seat.seat_people as sp on aac.keyID = sp.keyID " +
               "WHERE sp.personID in ( " +
               "SELECT _sp.personID " +
               "FROM seat.account_apikeyinfo_characters as _aac " +
               "INNER JOIN seat.account_apikeyinfo as _aa ON _aac.keyID = _aa.keyID " +
               "LEFT JOIN seat.seat_people as _sp on _aac.keyID = _sp.keyID " +
...
function getAllShipInfo(callback) {
    var query = function(connection) {
        return "SELECT it.typeID, it.typeName, ig.groupName, ie.notes, ie.insurancePrice, ie.insurancePayout, ie.srpPercentage " + 
