...
function fetchNew(settings, callback) {
  zkbKill.count({}, function(err, c) {
    if (err) return callback(err);
    if (c == 0) initialFetch(settings, callback);
    else fetchNewer(settings, callback);
  });

}

function fetchNewer(settings, callback) {
  var beginDate = new Date();
  beginDate.setMonth(beginDate.getMonth()-2);
  var corpId = settings['corp-id'];
  debugger;

  zkbKill.find({}).sort('-time').limit(1).select('kmId').exec(function(err, result) {
    if (err) return callback(err);
    if (result && result.length == 1) {
      var kmId = result[0].kmId;
      var modifier = '/afterKillID/' + kmId + '/';
      fetchPage(corpId, modifier, 1, callback);
    } else {
...
function initialFetch(settings, callback) {
  var beginDate = new Date();
  beginDate.setMonth(beginDate.getMonth()-2);
  var corpId = settings['corp-id'];
...
function fetchPage(corpId, modifier, page, callback) {
  zkbConnect.getData('losses/corporation/' + corpId + modifier
    ...
    for (var i = 0; i < len; ++i) {
      killMails.push({
        user: data[i].victim.characterID,
        kmId: data[i].killID,
    ...
    zkbKill.create(killMails, function(err) {
      if (err) return callback(err);
      fetchPage(corpId, modifier, page + 1, callback);
    });
  })
}
