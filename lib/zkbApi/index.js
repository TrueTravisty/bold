...
function getZbkLossesForCharacter(characterId, maxCount, skip, callback) {
  ...
  zkbKill.find({user: characterId, kill: false}).sort('-time').limit(maxCount).skip(skip).exec(function(err, result) {
...
function getToppKillList(maxCount, days, kills, callback) {
  ...
  zkbKill.find({kill: kills, time: {$gte: min}}).sort('-value').limit(maxCount).exec(function(err, result) {
    returnKills(err, result, callback);
  });
}
