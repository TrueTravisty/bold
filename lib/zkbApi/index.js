...
function getZbkKillsForCharacter(characterId, maxCount, callback) {
  zkbKill.find({user: characterId}).sort('-time').limit(maxCount).exec(function(err, result) {
    if (err) return callback(err);
    var kills = [];
...
exports.getZbkKillsForCharacter = getZbkKillsForCharacter;
